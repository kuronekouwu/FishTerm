...
export class SSHShellMonitor extends EventEmitter {
    ...
    private readonly commands: Record<string, any[]> = {
        ...
        network: ["NEW_RX=$( cat /sys/class/net/eth0/statistics/rx_bytes );   NEW_TX=$( cat /sys/class/net/eth0/statistics/tx_bytes ); echo \"RX:$((NEW_EX)) TX:$((NEW_TX))\"", networkFilter],
    ...
    closeAllMonitor(){
        for(const key of this.eventName){
